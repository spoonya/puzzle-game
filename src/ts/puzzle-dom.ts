...
export default class PuzzleDOM {
  ...
  private _puzzlePreview: HTMLElement | null =
    document.querySelector(".puzzle-preview");
  ...
  private _createRandArray(
    arrLength: number,
    isForInitScreen = false
  ): number[] {
    const arr: number[] = [];
  ...
  private _createPreview(): void {
    if (!this._puzzlePreview || !this.img) return;

    this._puzzlePreview.style.background = `url('${this.img}')`;
    this._puzzlePreview.style.backgroundSize = "cover";
    this._puzzlePreview.style.width = "150px";
    this._puzzlePreview.style.height = "150px";
    this._puzzlePreview.style.border = "2px solid #333";
    this._puzzlePreview.style.borderRadius = "5px";
    this._puzzlePreview.style.margin = "10px auto";
  }

  public createPuzzle(puzzleStyle: boolean, objForLoad?: any): void {
    this._cleanBoard();

    if (this._puzzlePreview) {
      this._puzzlePreview.style.display = puzzleStyle ? "block" : "none";
    }
