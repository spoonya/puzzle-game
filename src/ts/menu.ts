...
export default class Menu {
  ...
  private _puzzleStyle: boolean = false;
  ...
  public fillRecordsStorage(curSave?: any): void {
    ...
    if (!style()) {
      switch (record.boardSizeInt) {
        case 3:
          record.score = Math.ceil(100000 / ((record.timeInt / 1000 * 3.2) + record.moves * 3)).toString();
          break;

        case 4:
          record.score = Math.ceil(300000 / ((record.timeInt / 1000 * 1.75) + record.moves * 1.7)).toString();
          break;

        case 5:
          record.score = Math.ceil(550000 / ((record.timeInt / 1000 * 1.35) + record.moves * 1.5)).toString();
          break;

        case 6:
          record.score = Math.ceil(800000 / ((record.timeInt / 1000 * 1.1) + record.moves * 1.25)).toString();
          break;

        case 7:
          record.score = Math.ceil(1100000 / ((record.timeInt / 1000 * .8) + record.moves * 1.1)).toString();
          break;

        case 8:
          record.score = Math.ceil(1400000 / ((record.timeInt / 1000 * .5) + record.moves * 1)).toString();
          break;
      ...
      switch (record.boardSizeInt) {
        case 3:
          record.score = Math.ceil(300000 / ((record.timeInt / 1000 * 2) + record.moves * 2.7)).toString();
          break;

        case 4:
          record.score = Math.ceil(600000 / ((record.timeInt / 1000 * 1.5) + record.moves * 1.6)).toString();
          break;

        case 5:
          record.score = Math.ceil(1650000 / ((record.timeInt / 1000 * 1.2) + record.moves * 1.4)).toString();
          break;

        case 6:
          record.score = Math.ceil(2400000  / ((record.timeInt / 1000 * .85) + record.moves * 1.2)).toString();
          break;

        case 7:
          record.score = Math.ceil(3300000 / ((record.timeInt / 1000 * .5) + record.moves * 1.1)).toString();
          break;

        case 8:
          record.score = Math.ceil(4200000 / ((record.timeInt / 1000 * .3) + record.moves * 1)).toString();
          break;
  ...
  public createMenu(): void {
    ...
    const createMenuMain = (): void => {
      ...
      this.menuMainBtnArr.filter(el => el.id === 'btn-new')[0].addEventListener('click', () => {
        ...
        puzzleLogic.newGame(parseInt(this._menuItemSelect?.value), this._puzzleStyle);
      });
      ...
      this.menuMainBtnArr.filter(el => el.id === 'btn-load')[0].addEventListener('click', () => {
        const fillSavesTable = () => {
          ...
          for (let i = 0; i < this._savesArr.length; i++) {
            ...
            this._menuTableBtn.addEventListener('click', () => {
              this._savesArr.splice(this._savesArr.indexOf(this._savesArr.find((el) => el.id == this._savesArr[i].id)), 1);

              window.localStorage.setItem('saves', JSON.stringify(this._savesArr));

              this._saves = window.localStorage.getItem('saves');

              new Modal(Modal.alertType.onSaveDelete).showAlert().closeAlert(2000);
      ...
      this.menuMainBtnArr.filter(el => el.id === 'btn-save')[0].addEventListener('click', () => {
        ...
        const generateRandId = () => {
          ...
          randId = Math.floor(Math.random() * (999999 + 1)).toString() + Math.random().toString(36).substr(2, 5).toString();
    ...
    const createMenuSettings = (): void => {
      ...
      for (let i = 0; i < menuSettingsSetup.puzzleStyle.option.length; i++) {
        ...
        this._menuItemInput = document.createElement('input');
        this._menuItemLabel = document.createElement('label');
        setAttributes(this._menuItemInput, menuSettingsSetup.puzzleStyle.option[i].input);
        setAttributes(this._menuItemLabel, menuSettingsSetup.puzzleStyle.option[i].label);

        if (menuSettingsSetup.puzzleStyle.option[i].input.checked) {
          this._puzzleStyle = menuSettingsSetup.puzzleStyle.option[i].input.value;
        }

        this._menuItemLabel.innerText = menuSettingsSetup.puzzleStyle.option[i].txt;
