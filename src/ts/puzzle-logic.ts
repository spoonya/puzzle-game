import { header, menu, puzzleDOM } from "./index";
...
export default class PuzzleLogic {
  ...
  private _move(item: HTMLElement): void {
    ...
    const chooseAnime = () => {
      ...
      if (
        coords.itemCoords.col === coords.emptyCoords.col &&
    ...
    if (
      Math.abs(coords.itemCoords.col - coords.emptyCoords.col) +
        Math.abs(coords.itemCoords.row - coords.emptyCoords.row) >
      1
    ) {
      return;
    } else {
  ...
  private _isSolved(): boolean {
    ...
    return (
      JSON.stringify(puzzleDOM.solvedNumArr) ===
      JSON.stringify(this.unsolvedNumArr)
    );
  }
